...
@section('title', $activeCategory->title ?? 'Каталог')

@section('content')
{{-- @dd(request()) --}}
    <!-- Breadcrumbs -->
    <ul class="flex flex-wrap mb-6 breadcrumbs gap-y-1 gap-x-4">
        <li><a href="{{ route('home') }}" class="text-xs text-body hover:text-pink">Главная</a></li>
        <li><a href="{{ route('catalog') }}" class="text-xs text-body hover:text-pink">Каталог</a></li>
    ...
    <section>
        ...
        <!-- Categories -->
        <div class="grid grid-cols-2 gap-3 mt-8 sm:grid-cols-3 xl:grid-cols-5 sm:gap-4 md:gap-5">
    ...
    <section class="mt-16 lg:mt-24">
        ...
        <div class="flex flex-col gap-12 mt-8 lg:flex-row lg:gap-6 2xl:gap-8">

            <!-- Filters -->
            <aside class="basis-2/5 xl:basis-1/4">
                <form action="{{ route('catalog', $activeCategory) }}" class="overflow-auto max-h-[320px] lg:max-h-[100%] space-y-10 p-6 2xl:p-8 rounded-2xl bg-card">
                    ...
                    @if(request('filters'))
                        <div>
                            <a href="{{ route('catalog', $activeCategory) }}" class="w-full !h-16 btn btn-outline">Сбросить фильтры</a>
                        </div>
                    @endif
                </form>
            </aside>
            ...
            <div class="basis-auto xl:basis-3/4">
                ...
                <div class="flex flex-col justify-between gap-4 mb-8 md:flex-row md:items-center">
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2">
							<form>
								<input type="hidden" name="catalog_view" value="tile">
								<button type="submit" class="inline-flex items-center justify-center w-10 h-10
								@if (session('catalog_view', 'tile') !== 'tile') text-white @else text-pink @endif rounded-md bg-card hover:text-pink">
							...
							<form>
								<input type="hidden" name="catalog_view" value="list">
								<button type="submit" class="inline-flex items-center justify-center w-10 h-10 
								@if (session('catalog_view') === 'list') text-pink @else text-white @endif
								rounded-md bg-card hover:text-pink">
									<svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 52 52">
                    ...
                    <div x-data="{}" class="flex flex-col gap-3 sm:flex-row sm:items-center">
                        <span class="text-body text-xxs sm:text-xs">Сортировать по</span>
                        <form x-ref="sortForm" action="{{ route('catalog', $activeCategory) }}">
                            <select
                                name="sort"
                ...
                <!-- Products list -->
                <div class="grid 
				@if (session('catalog_view', 'tile') === 'tile')
					sm:grid-cols-2 xl:grid-cols-3 
				@endif
				grid-cols-1 products gap-x-6 2xl:gap-x-8 gap-y-8 lg:gap-y-10 2xl:gap-y-12">
