...
@section('content')
    <x-forms.auth-form title="Регистрация" action="{{route('store')}}" method="POST">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
                <x-forms.text-input type="password" name="password" placeholder="Пароль" required='true'
        ...
        <x-slot:links>
            <div class="text-xxs md:text-xs">Есть аккаунт? <a href="{{ route('login') }}"
                    class="text-white hover:text-white/70 font-bold underline underline-offset-4">Войти</a></div>
        </x-slot:links>
    </x-forms.auth-form>
@endsection
