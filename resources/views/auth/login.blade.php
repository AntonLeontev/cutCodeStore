...
@section('content')
    <x-forms.auth-form title="Вход в аккаунт" action="{{route('sign-in')}}" method="POST">
        <x-forms.text-input name="email" type="email" placeholder="E-mail" required='true' :isError="$errors->has('email')" value="{{old('email')}}" />
        @error('email')
            <x-forms.error>{{ $message }}</x-forms.error>
        @enderror
        <x-forms.text-input name="password" type="password" placeholder="Пароль" required='true' />
