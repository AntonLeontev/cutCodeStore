			...
			<div class="flex items-center justify-between header-inner lg:justify-start">
				<div class="header-logo shrink-0">
					<a href="{{route('home')}}" rel="home">
						<img src="/logo.svg" class="w-[120px] xs:w-[148px] md:w-[201px] h-[30px] xs:h-[36px] md:h-[50px]" alt="CutCode">
					</a>
				</div><!-- /.header-logo -->
				<div class="items-center hidden gap-8 ml-8 mr-8 header-menu grow lg:flex">
					<form action="{{route('catalog', $activeCategory ?? '')}}" class="hidden gap-3 lg:flex">
						<input type="search" name="search" class="w-full h-12 px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs shadow-transparent outline-0 transition" value="{{request('search')}}" placeholder="Поиск..." required>
				...
				<div class="flex items-center gap-3 header-actions md:gap-5">
					...
					@elseauth
						<div x-data="{dropdownProfile: false}" class="relative profile">
							<button @click="dropdownProfile = ! dropdownProfile" class="flex items-center text-white transition hover:text-pink">
								<span class="sr-only">Профиль</span>
								<img src="{{auth()->user()->avatar}}" class="rounded-full shrink-0 w-7 md:w-9 h-7 md:h-9" alt="{{auth()->user()->name}}">
								<span class="hidden ml-2 font-medium md:block">{{auth()->user()->name}}</span>
								<svg class="w-3 h-3 ml-2 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 30 16">
							...
							>
								...
								<div class="flex items-center mt-3">
									<img src="{{auth()->user()->avatar}}" class="rounded-full w-11 h-11" alt="{{auth()->user()->name}}">
									<span class="ml-3 text-xs font-bold md:text-sm">{{auth()->user()->name}}</span>
								</div>
								<div class="mt-4">
									<ul class="space-y-2">
										<li><a href="#" class="text-xs font-medium text-body hover:text-white">Мои заказы</a></li>
										<li><a href="#" class="text-xs font-medium text-body hover:text-white">Редактировать профиль</a></li>
									</ul>
								</div>
								<div class="mt-6">
									<div class="inline-flex items-center cursor-pointer text-body hover:text-pink">
										...
										<form action="{{route('logout')}}" method="POST">
											@csrf
											@method('DELETE')
											<input class="ml-2 font-medium" type="submit" value="Выйти">
										</form>
									</div>
								</div>
							</div>
					...
					<a href="#" class="flex items-center gap-3 text-pink hover:text-white">
						<svg xmlns="http://www.w3.org/2000/svg" class="w-6 md:w-7 md:h-7" fill="currentColor" viewBox="0 0 52 52">
						...
						<div class="flex-col hidden gap-2 sm:flex">
							<span class="leading-none text-body text-xxs">3 шт.</span>
